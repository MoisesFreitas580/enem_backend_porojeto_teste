...
export async function seedKnowledgeObjects() {
  const data: Array<{
    areaCode: AreaCode;
    discipline: string;
    objects: string[];
    ...
    },
    {
      ...
      objects: [
        ...
        'Materiais, suas propriedades e usos - Propriedades de materiais. Estados físicos de materiais. Mudanças de estado. Misturas: tipos e métodos de separação. Substâncias químicas: classificação e características gerais. Metais e Ligas metálicas. Ferro, cobre e alumínio. Ligações metálicas. Substâncias iônicas: características e propriedades. Substâncias iônicas do grupo: cloreto, carbonato, nitrato e sulfato. Ligação iônica. Substâncias moleculares: características e propriedades. Substâncias moleculares: H , O , 2 2 N , Cl , NH , H O, HCl, CH . Ligação Covalente. Polaridade de moléculas. Forças 2 2 3 2 4 intermoleculares. Relação entre estruturas, propriedade e aplicação das substâncias.',
  ...
  // mapa: (areaCode -> areaId)
  const areas = await prisma.area.findMany();
  const areaIdByCode = new Map(areas.map((a) => [a.code, a.id]));

  for (const row of data) {
    const areaId = areaIdByCode.get(row.areaCode);
    if (!areaId) throw new Error(`Area não encontrada: ${row.areaCode}`);

    // disciplina (unique: [areaId, name])
    const discipline = await prisma.discipline.upsert({
      where: { areaId_name: { areaId, name: row.discipline } },
      update: {},
      create: { areaId, name: row.discipline },
    });

    // knowledge objects (unique: [disciplineId, name])
    for (const objName of row.objects) {
      await prisma.knowledgeObject.upsert({
        where: {
          disciplineId_name: { disciplineId: discipline.id, name: objName },
        },
        update: {},
        create: { disciplineId: discipline.id, name: objName },
      });
    }
  }

  const count = await prisma.knowledgeObject.count();
  console.log(`seedKnowledgeObjects ---OK (total knowledge objects: ${count})`);
}
